				...
				case 5: {
					...
					if (mes_fin == last_topologie+1) { //daca toate nodurile din topologie au finalizat procesarea mesajelor de la timpul curent
						...
						if (((timp+1)%2==0) && (timp+1 < 2*nr_ev)) {  //daca mai am evenimente, le anunt nodurilor direct implicate
							mesaj.msg_type = 7; //anunt eveniment
						...
						if ((nr_msg == 0) && (timp >= 2*nr_ev)) {//daca nu s-au primit mesaje (din protocolul linkstate) in aceasta runda si nu mai sunt evenimente
							mesaj.msg_type = 9; //s-a terminat simularea
						}	
						else {
							mesaj.msg_type = 6; //se da liber la procesare mesaje vechi (primite la t-1) din coada
							mes_fin = 0;
							nr_msg = 0;
							timp ++;
						}
